...
class Contract(models.Model):
    STATUS_CHOICES = [
        ("DRAFT", "Draft"),
        ("OPEN", "Open for Bids"),                   # 🆕 For the open market flow
        ("PROPOSED", "Proposed (Pending Review)"),   # 🆕 Direct proposal to a specific user
        ("AWAITING_FUNDING", "Awaiting Funding"),
        ("FUNDED", "Funded & Active"),
        ("DELIVERED", "Delivered - Awaiting Inspection"),
        ("DISPUTED", "In Dispute"),                       
        ("RELEASED", "Completed"),
        ("REJECTED", "Rejected/Cancelled"),          # 🆕 To tombstone dead contracts
    ]
    ...
    # Counterparty details (Nullable for "Open" contracts)
    vendor = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="vendor_contracts")
    vendor_email = models.EmailField(blank=True, null=True) # Fallback for unregistered invites
    
    # Visibility
    is_public = models.BooleanField(default=False) # 🆕 True if it goes to the open market
    
    # Terms (This acts as the "Current/Active" state)
    item_title = models.CharField(max_length=255)
    item_description = models.TextField()
    item_amount = models.DecimalField(max_digits=12, decimal_places=2)
    delivery_fee = models.DecimalField(max_digits=10, decimal_places=2)
    delivery_days = models.IntegerField(default=2, help_text="Estimated days for delivery")
    total_escrow = models.DecimalField(max_digits=12, decimal_places=2, blank=True)
    plain_language_summary = models.TextField(blank=True, null=True) # Generated by LLM later
    ...
    accepted_at = models.DateTimeField(null=True, blank=True) # 🆕 Required for your business rule tests
    ...
    def save(self, *args, **kwargs):
        ...
        # If bound to a vendor, it is no longer public
        if self.vendor and self.status not in ["DRAFT", "OPEN"]:
    ...
    def clean(self):
        ...
        super().clean()
        # If it is a direct contract (not public), a counterparty MUST be specified
        if not self.is_public and not self.vendor and not self.vendor_email:
            raise ValidationError({
                "vendor": "A vendor or vendor email must be provided for private contracts."
            })
...
class ContractApplication(models.Model):
    """Handles the 'Open Market' bids from vendors."""
    contract = models.ForeignKey(Contract, on_delete=models.CASCADE, related_name="applications")
    applicant = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="contract_applications")
    cover_message = models.TextField(blank=True)
    proposed_amount = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True) # If they can counter-offer
    ...
    class Meta:
        unique_together = ('contract', 'applicant') # A user can only apply once per contract


class ContractVersion(models.Model):
    """The immutable history log. Every time terms change, a new version is stamped."""
    contract = models.ForeignKey(Contract, on_delete=models.CASCADE, related_name="versions")
    version_number = models.PositiveIntegerField()
    
    # Snapshot of the terms at this version
    item_title = models.CharField(max_length=255)
    item_description = models.TextField()
    item_amount = models.DecimalField(max_digits=12, decimal_places=2)
    delivery_fee = models.DecimalField(max_digits=10, decimal_places=2)
...
class AdminRole(models.TextChoices):
    TIER_1_SUPPORT = 'TIER_1', 'Tier 1 Support (Read Only / Basic)'
    DISPUTE_MANAGER = 'DISPUTE_MANAGER', 'Dispute Manager (Can resolve contracts)'
    RISK_OFFICER = 'RISK_OFFICER', 'Risk Officer (Can suspend/edit trust scores)'
    SUPER_ADMIN = 'SUPER_ADMIN', 'Super Admin (Full Access)'


class AdminProfile(models.Model):
    """
    Separates admin privileges from standard user data.
    Only users with an AdminProfile can access the back-office.
    """
    ...
    role = models.CharField(max_length=30, choices=AdminRole.choices)
    is_active = models.BooleanField(default=True)
